...
# Modified from https://github.com/lucidrains/video-diffusion-pytorch/blob/main/video_diffusion_pytorch/video_diffusion_pytorch.py
...
def Upsample(dim,padding=None):
    if padding is None:
        padding = (0,1,1)
    return nn.ConvTranspose3d(dim, dim, (1, 4, 4), (1, 2, 2), padding)

def Downsample(dim,padding=None):
    if padding is None:
        padding = (0, 1, 1)
    return nn.Conv3d(dim, dim, (1, 4, 4), (1, 2, 2), padding)
...
class Unet3D_Trajectory(nn.Module):
    def __init__(
        self,
        dim,
        cond_dim = None,
        ...
        if self.args.dataset!='languagetable':
            ...
            self.embed_state = Mlp(in_features=(args.num_frames-1)*self.state_dim, hidden_features = cond_dim*4, out_features=cond_dim, act_layer=approx_gelu, drop=0)
        else:
            self.state_dim = 2
            approx_gelu = lambda: nn.GELU(approximate="tanh")
            self.embed_state = Mlp(in_features=(args.num_frames-1)*self.state_dim, hidden_features = cond_dim*4, out_features=cond_dim, act_layer=approx_gelu, drop=0)
        # self.mask_emb_fn = nn.Embedding(num_embeddings=1, embedding_dim=cond_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            if ind == 1 and self.args.video_size == [288,512]:
                self.downs.append(nn.ModuleList([
                    ...
                    Downsample(dim_out,padding=(0,2,1)) if not is_last else nn.Identity()
                ]))
            else:
                self.downs.append(nn.ModuleList([
                    block_klass_cond(dim_in, dim_out),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            if ind == 1 and self.args.video_size == [288,512]:
                self.ups.append(nn.ModuleList([
                    ...
                    Upsample(dim_in,padding=(0,2,1)) if not is_last else nn.Identity()
                ]))
            else:
                self.ups.append(nn.ModuleList([
                    block_klass_cond(dim_out * 2, dim_in),
    ...
    def forward(
        ...
        if self.has_cond:
            ...
            state_embeddings = self.embed_state(cond)
            mask = prob_mask_like((batch,), null_cond_prob, device = device)
            cond = torch.where(rearrange(mask, 'b -> b 1'), self.null_cond_emb, state_embeddings)
...
def video_to_tensor(path, channels=3, transform=None):
    # 设置默认的转换操作，如果没有提供特定的transform
    if transform is None:
        transform = T.ToTensor()
    # 使用imageio读取视频
    reader = imageio.get_reader(path)

    # 读取每一帧，并将其转换为PIL图像，然后应用transform
    tensors = []
    for frame in reader:
        # 将imageio的数组转换为PIL图像
